...
class Intersection:
    ...
    def __init__(self,ox,oy,x,y, isHighway):
        self.ox = ox
        self.oy = oy
        self.road=[]
        self._n = len(x)
    ...
    def findL(self,r):
        ...
        for i in range(self._n):
            ...
            if(self.isHighway and ti < math.pi/2.0):
                continue
            if(ti<tmin):
                tmin=ti
                imin=i
    ...
    def plot(self,d,r):
        ...
        #绘制第i条弧线,位于第i点与i-1之间
        for i in range(self._n):
            if abs(math.pi-self.theta[i])>0.08:#不为直线
                #旋转方向:
                xx = []
                yy = []
                ...
                if(self.isHighway and self.theta[i] < math.pi/2.0):
                    xx.append(self.road[i-1][0]+d*p[0])
                    yy.append(self.road[i-1][1]+d*p[1])
                    xx.append(self.ox + d/math.tan(self.theta[i]/2.0)*self.road[i-1][2] + d*p[0])
                    yy.append(self.oy + d/math.tan(self.theta[i]/2.0)*self.road[i-1][3] + d*p[1])
